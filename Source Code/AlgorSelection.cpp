...
void AlgorSelection::renderAlgorRecord(int index, SortingAlgor data) {
...
void AlgorSelection::renderAlgorList(vector<SortingAlgor> datas, bool* selectedAlgors, size_t size, bool haveSelected) {
	if (haveSelected) {
		cout << "\n\nDANH SÁCH CÁC THUẬT TOÁN ĐÃ LỰA CHỌN: " << endl;
		Decorator::renderAlgorsHeader();
		for (int i = 0; i < size; ++i) {
			if (selectedAlgors[i]) {
				AlgorSelection::renderAlgorRecord(i, datas[i]);
			}
		}
		Decorator::drawHr(85);
	}
...
bool AlgorSelection::renderMainView(bool selectedAlgors[]) {
	...
	do {
		AlgorSelection::renderAlgorList(datas, selectedAlgors, size, 0);
		if (haveDisplaySelected) {
			AlgorSelection::renderAlgorList(datas, selectedAlgors, size, 1);
		}
